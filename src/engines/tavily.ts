import { loadConfig } from '../config.js';
import type { Engine, ScrapeOptions, ScrapeResult, Attempt, Capability } from '../types.js';

/**
 * Tavily — paid API for content extraction.
 * POST to https://api.tavily.com/extract with URLs to get clean text back.
 * Good at extracting structured content from protected/complex pages.
 */
export function createTavilyEngine(): Engine {
  return {
    name: 'tavily',
    tier: 2,
    capabilities: ['html', 'search'] as Capability[],

    async isAvailable(): Promise<boolean> {
      const config = loadConfig();
      return !!config.tavilyApiKey;
    },

    async scrape(url: string, opts: ScrapeOptions): Promise<ScrapeResult | null> {
      const start = Date.now();
      const attempts: Attempt[] = [];
      const config = loadConfig();
      const timeout = opts.timeout ?? 30_000;

      if (!config.tavilyApiKey) {
        attempts.push({
          engine: 'tavily',
          success: false,
          error: 'No TAVILY_API_KEY configured',
          durationMs: Date.now() - start,
        });
        return null;
      }

      try {
        const controller = new AbortController();
        const timer = setTimeout(() => controller.abort(), timeout);

        const response = await fetch('https://api.tavily.com/extract', {
          method: 'POST',
          headers: {
            'Content-Type': 'application/json',
          },
          body: JSON.stringify({
            api_key: config.tavilyApiKey,
            urls: [url],
          }),
          signal: controller.signal,
        });

        clearTimeout(timer);

        if (!response.ok) {
          const errorText = await response.text().catch(() => 'Unknown error');
          attempts.push({
            engine: 'tavily',
            success: false,
            error: `HTTP ${response.status}: ${errorText}`,
            durationMs: Date.now() - start,
          });
          return null;
        }

        const data = (await response.json()) as {
          results?: Array<{
            url: string;
            raw_content?: string;
            title?: string;
          }>;
        };

        const result = data.results?.[0];
        if (!result?.raw_content?.trim()) {
          attempts.push({
            engine: 'tavily',
            success: false,
            error: 'Tavily returned empty result',
            durationMs: Date.now() - start,
          });
          return null;
        }

        const markdown = result.raw_content;

        attempts.push({
          engine: 'tavily',
          success: true,
          durationMs: Date.now() - start,
          contentLength: markdown.length,
        });

        return {
          url,
          markdown,
          title: result.title ?? undefined,
          engine: 'tavily',
          attempts,
          timestamp: new Date().toISOString(),
          durationMs: Date.now() - start,
        };
      } catch (err) {
        const message = err instanceof Error ? err.message : String(err);
        attempts.push({
          engine: 'tavily',
          success: false,
          error: message,
          durationMs: Date.now() - start,
        });
        return null;
      }
    },
  };
}
